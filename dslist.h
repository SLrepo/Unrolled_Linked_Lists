...
template <class T> Node::Node(const T& v){
  next_=NULL;
  prev_=NULL;
  num_elements=1;
  elements[0]=v;
...
public:
  ...
  T*& operator*()  { return ptr_->elements;  }

  // increment & decrement operators
  list_iterator<T> & operator++() { // pre-increment, e.g., ++iter
    if (offset==NUM_ELEMENTS_PER_NODE-1){
      ptr_ = ptr_->next_; 
      offset=0;
    }
    else {
  ...
  list_iterator<T> & operator--() { // pre-decrement, e.g., --iter
    if (offset==0){
      ptr_ = ptr_->prev_; 
      offset=NUM_ELEMENTS_PER_NODE-1;
    }
    else {
      offset--;
    }
...
class Unrolled {
public:
  Unrolled() : head_(NULL), tail_(NULL), size_(0) {}
  Unrolled(const Unrolled<T>& old) { this->copy_list(old); }
  ~Unrolled() { this->destroy_list(); }
  Unrolled& operator= (const Unrolled<T>& old);
  ...
  const T& back() const { return tail_->elements[tail->num_elements-1]; }
  T& back() { return tail_->[tail->num_elements-1]; }
...
private:
  void copy_list(Unrolled<T> const & old);
  void destroy_list();
...
template <class T>
Unrolled<T>& Unrolled<T>::operator= (const Unrolled<T>& old) {
  if (&old != this) {
    this->destroy_list();
    this->copy_list(old);
  }
...
template <class T> 
void Unrolled<T>::push_back(const T& v) {
  ...
  // normal case:  at least one node already
    ...
    if(tail_->num_elements<NUM_ELEMENTS_PER_NODE){
      tail->num_elements++;
      tail_->elements[num_elements-1]=v;
      delete [] newp;
    }
    else{
    newp -> prev_ = tail_;
...
template <class T> 
void Unrolled<T>::push_front(const T& v) {
  Node<T>* newp= new Node<T>(v);
  if (!tail_)
    head_=tail_=newp;
  else{
...
template <class T> 
typename Unrolled<T>::iterator Unrolled<T>::erase(iterator itr) {
  assert (size_ > 0);
  --size_;
  ...
  // Normal remove
  else if (itr.prt_->num_elements==1){
...
template <class T> 
typename Unrolled<T>::iterator Unrolled<T>::insert(iterator itr, T const& v) {
  ++size_ ;
  if (itr.offset==0){
    ...
    Node<T>* p=new Node<T>(itr.ptr->elements[NUM_ELEMENTS_PER_NODE-1]);
    p->next_=itr.ptr_->next;
    p->prev_=itr.ptr_;
    ...
    if (itr.offset!=NUM_ELEMENTS_PER_NODE-1){
      for(unsigned int i=itr.offset; i <NUM_ELEMENTS_PER_NODE-1; i++){
        itr.ptr_->elements[i+1]=itr.ptr_->elements[i];
      }
    }
    itr.ptr_->elements[offset]=v;
    return itr;
  }
...
template <class T> 
void Unrolled<T>::copy_list(Unrolled<T> const & old) {
  size_ = old.size_;
